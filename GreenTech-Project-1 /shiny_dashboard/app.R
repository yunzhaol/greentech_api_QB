...
# Priority: 1. Credentials file, 2. Environment variable, 3. Default location
...
if (file.exists(BQ_CREDENTIALS_FILE)) {
...
} else if (GOOGLE_APPLICATION_CREDENTIALS != "" && file.exists(GOOGLE_APPLICATION_CREDENTIALS)) {
  # Use environment variable if set and file exists
  cat("Using BigQuery credentials from environment variable:", GOOGLE_APPLICATION_CREDENTIALS, "\n")
} else {
...
get_new_clients <- function() {
  ...
  query <- sprintf("
    ...
    WHERE EXTRACT(YEAR FROM first_txn_date) = EXTRACT(YEAR FROM CURRENT_DATE())
      AND EXTRACT(MONTH FROM first_txn_date) = EXTRACT(MONTH FROM CURRENT_DATE())
...
get_returning_clients <- function() {
  ...
  query <- sprintf("
    ...
    FROM first_transactions
    WHERE EXTRACT(YEAR FROM first_txn_date) < EXTRACT(YEAR FROM CURRENT_DATE())
       OR (EXTRACT(YEAR FROM first_txn_date) = EXTRACT(YEAR FROM CURRENT_DATE())
           AND EXTRACT(MONTH FROM first_txn_date) < EXTRACT(MONTH FROM CURRENT_DATE()))
  ", BQ_PROJECT, BQ_DATASET, BQ_PROJECT, BQ_DATASET)

...
def create_service_item(name: str = "Service", description: str = "General Service") -> Dict:
    """
    Creates a service item in QuickBooks.
    
    Args:
        name: Item name (default: "Service")
        description: Item description
        
    Returns:
        Created item dict with Id
    """
    # First, try to find an income account
    # Query for common income account names
    account_query = "SELECT * FROM Account WHERE AccountType = 'Income' MAXRESULTS 1"
    try:
        account_response = _make_request("GET", "query", params={"query": account_query})
        account_list = account_response.get("QueryResponse", {}).get("Account", [])
        if account_list and not isinstance(account_list, list):
            account_list = [account_list]
        
        income_account = account_list[0] if account_list else None
    except:
        income_account = None
    ...
    # Add income account if found
    if income_account:
        payload["IncomeAccountRef"] = {
            "value": income_account.get("Id")
        }
    else:
        # Try with account name as fallback
        payload["IncomeAccountRef"] = {
            "name": "Services"
        }
    
    response = _make_request("POST", "item", data=payload)
    return response.get("Item", {})

def get_or_create_service_item(name: str = "Service") -> Dict:
    """
    Gets an existing service item or creates it if it doesn't exist.
    ...
    items = query_items(name)
    if items:
        return items[0]
    
    # Create if not found
    return create_service_item(name)
